...
| Function prefix | agentic_admin_ (blocked) | agentic_admin_ (passes) |
...
1. `agentic-admin.php`
   ...
   - Constant: `define( 'AGENTIC_ADMIN_VERSION', '0.10.0' )` → `'0.11.0'`
...
### Conflict 1: `agentic-admin.php` (#206 ↔ #198)
...
Also: #199's new functions (`agentic_admin_sensitive_columns`, `agentic_admin_redact_sensitive_row`) need the rename applied manually because #206 didn't see them yet. All `'agentic-admin'` text-domain strings introduced by #199 → `'agentic-admin'`.
...
- [ ] Verify `agentic-admin.zip` build via `npm run dist` does NOT contain `CLOUDFEST_HACKATHON.md` or `docs/FEEDBACK-DEV.md`
