...
const ChatContainer = ({ modelReady = false, isLoading = false, setIsLoading }) => {
    ...
    const handleConfirm = useCallback(() => {
        if (pendingConfirmation) {
            ...
            if (pendingConfirmation.isIntentConfirmation && pendingConfirmation.label) {
    ...
    const copyAllConversation = useCallback(async () => {
            ...
            .map((msg) => {
                ...
                switch (msg.type) {
                    ...
                    case 'ability_request':
                        prefix = 'Tool Request:';
                        content = `${msg.abilityLabel || msg.abilityName}\nInput: ${JSON.stringify(msg.input, null, 2)}`;
                        break;
                    case 'ability_result':
                        prefix = 'Tool Result:';
                        content = `${msg.abilityName}\nSuccess: ${msg.success}\nResult: ${typeof msg.result === 'object' ? JSON.stringify(msg.result, null, 2) : msg.result}`;
                        break;
                    case 'error':
                        prefix = 'Error:';
        ...
        try {
            ...
            console.error('[ChatContainer] Failed to copy conversation:', error);
        }
    }, [messages]);
            ...
            <div className="wp-agentic-admin-chat-header">
                <div className="wp-agentic-admin-chat-header__actions">
                    <Button
                        ...
                        disabled={messages.length === 0 || isLoading || isStreaming || isExecutingTool}
                    >
                        Copy All
                    </Button>
                    <Button
            ...
            {isExecutingTool && !isRunningWorkflow && (
                <div className="agentic-message agentic-message--loading">
                    ...
                    <div className="agentic-loading">
                        ...
                        <span className="agentic-loading__text">Running tool...</span>
                    </div>
                </div>
            )}
                ...
                >
                    {pendingConfirmation.isIntentConfirmation ? (
                        ...
                        <p className="wp-agentic-admin-intent-question">
                            Did you mean: <strong>{pendingConfirmation.label}</strong>?
                        </p>
                    ...
                    <div className="wp-agentic-admin-confirm-actions">
                        <Button variant="tertiary" onClick={handleCancel}>
                            {pendingConfirmation.isIntentConfirmation ? 'No' : 'Cancel'}
                        </Button>
                        <Button
                            variant="primary"
                            isDestructive={! pendingConfirmation.isIntentConfirmation}
