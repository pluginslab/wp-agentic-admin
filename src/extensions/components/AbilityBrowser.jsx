...
const AbilityCard = ({ ability, onExecute, isExecuting }) => {
    ...
    const hasInputSchema = ability.input_schema?.properties && 
    ...
    const renderSchemaHints = () => {
        ...
        return (
            <div className="wp-neural-admin-ability-card__schema">
                ...
                <ul>
                    {Object.entries(props).map(([key, schema]) => (
                        <li key={key}>
                            ...
                            {schema.default !== undefined && <span> = {JSON.stringify(schema.default)}</span>}
    ...
    return (
        <div className={`wp-neural-admin-ability-card ${isDestructive ? 'wp-neural-admin-ability-card--destructive' : ''}`}>
            <div className="wp-neural-admin-ability-card__header">
                <h4>{ability.label}</h4>
                {isDestructive && (
                    <span className="wp-neural-admin-badge wp-neural-admin-badge--warning">
                        Destructive
                    </span>
                )}
            </div>
            
            <p className="wp-neural-admin-ability-card__description">
                {ability.description}
            </p>

            <div className="wp-neural-admin-ability-card__id">
            ...
            {hasInputSchema && (
                <>
                    <button
                        type="button"
                        className="button button-link wp-neural-admin-ability-card__toggle"
                    ...
                    {showInput && (
                        <div className="wp-neural-admin-ability-card__input">
                            {renderSchemaHints()}
                            <textarea
                                value={inputJson}
                            ...
                            />
                            {inputError && (
                                <div className="wp-neural-admin-ability-card__error">
                                    {inputError}
                                </div>
                            )}
                        </div>
            ...
            <div className="wp-neural-admin-ability-card__actions">
                <button
                    type="button"
                    className={`button ${isDestructive ? 'button-link-delete' : 'button-primary'}`}
...
const ResultPanel = ({ result, onClear }) => {
    ...
    return (
        <div className={`wp-neural-admin-result-panel ${isError ? 'wp-neural-admin-result-panel--error' : 'wp-neural-admin-result-panel--success'}`}>
            <div className="wp-neural-admin-result-panel__header">
                <h4>
                    <span className={`dashicons ${isError ? 'dashicons-warning' : 'dashicons-yes-alt'}`} />
            ...
            <div className="wp-neural-admin-result-panel__content">
...
const AbilityBrowser = () => {
    ...
    const handleExecute = async (ability, input) => {
        ...
        try {
            const result = await abilitiesApi.executeAbilityById(ability.name, input);
    ...
    if (isLoading) {
        return (
            <div className="wp-neural-admin-ability-browser wp-neural-admin-ability-browser--loading">
                <Spinner />
    ...
    if (error) {
        return (
            <div className="wp-neural-admin-ability-browser wp-neural-admin-ability-browser--error">
    ...
    if (abilities.length === 0) {
        return (
            <div className="wp-neural-admin-ability-browser wp-neural-admin-ability-browser--empty">
                <div className="notice notice-warning">
                    <p>No abilities found. Make sure the Abilities API plugin is active and abilities are registered.</p>
                </div>
                <button type="button" className="button" onClick={loadAbilities}>
                    Refresh
                </button>
    ...
    return (
        <div className="wp-neural-admin-ability-browser">
            ...
            <div className="wp-neural-admin-ability-browser__grid">
