...
import { route, isWorkflowQuery, isConversationalQuery } from '../message-router';
...
describe('MessageRouter', () => {
	...
	describe('Conversational Routing (Questions)', () => {
		it('should route "what is" questions to conversational mode', () => {
			expect(route('what is a transient?').type).toBe('conversational');
			expect(route('what are plugins?').type).toBe('conversational');
			expect(route('what does cache flush do?').type).toBe('conversational');
		});

		it('should route "how" questions to conversational mode', () => {
			expect(route('how do I install a plugin?').type).toBe('conversational');
			expect(route('how can I optimize my database?').type).toBe('conversational');
			expect(route('how to clear cache?').type).toBe('conversational');
		});

		it('should route "why" questions to conversational mode', () => {
			expect(route('why is my site slow?').type).toBe('conversational');
			expect(route('why are plugins important?').type).toBe('conversational');
		});

		it('should route "explain" requests to conversational mode', () => {
			expect(route('explain what transients are').type).toBe('conversational');
			expect(route('tell me about WordPress cache').type).toBe('conversational');
			expect(route('describe the plugin system').type).toBe('conversational');
		});
	...
	describe('Workflow Routing (Keywords)', () => {
		it('should route workflow keywords to workflow mode', () => {
			const mockWorkflow = { id: 'site-cleanup', label: 'Full Site Cleanup' };
			...
			expect(workflowRegistry.detectWorkflow).toHaveBeenCalledWith('full site cleanup');
		});

		it('should route "check site performance" to workflow', () => {
			const mockWorkflow = { id: 'performance-check', label: 'Performance Check' };
			workflowRegistry.detectWorkflow.mockReturnValue(mockWorkflow);
	...
	describe('ReAct Routing (Default)', () => {
		...
		it('should route vague queries to ReAct mode', () => {
			...
			expect(route('how many plugins do I have?').type).toBe('react');
		});

		it('should route multi-action queries to ReAct mode', () => {
			expect(route('check my site and optimize if needed').type).toBe('react');
			expect(route('show errors and deactivate broken plugin').type).toBe('react');
			expect(route('clean up database and check health after').type).toBe('react');
		});
	});
	...
	describe('Edge Cases', () => {
		...
		it('should prioritize conversational over workflow', () => {
			...
			expect(route('what is a plugin audit?').type).toBe('conversational');
		});

		it('should be case-insensitive for question patterns', () => {
			expect(route('What Is A Plugin?').type).toBe('conversational');
			expect(route('HOW DO I CLEAR CACHE?').type).toBe('conversational');
		});
	});
