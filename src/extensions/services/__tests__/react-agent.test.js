...
describe('ReactAgent', () => {
		...
		it('should handle clean JSON output', async () => {
			mockEngine.chat.completions.create
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "tool_call", "tool": "wp-agentic-admin/plugin-list", "args": {}}',
						},
					}],
				})
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "final_answer", "content": "You have 1 plugin installed."}',
						},
					}],
				});
			...
			expect(result.toolsUsed).toEqual(['wp-agentic-admin/plugin-list']);
		});
		...
		it('should sanitize control characters in JSON', async () => {
			...
			mockEngine.chat.completions.create
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "tool_call", "tool": "wp-agentic-admin/plugin-list", "args": {"reason": "List\nplugins"}}',
						},
					}],
				})
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "final_answer", "content": "Done"}',
						},
					}],
				});
		...
		it('should extract first JSON object when multiple are output', async () => {
			...
			mockEngine.chat.completions.create
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "tool_call", "tool": "wp-agentic-admin/plugin-list", "args": {}}{"action": "final_answer", "content": "Done"}',
						},
					}],
				})
				.mockResolvedValueOnce({
	...
	describe('Tool Execution', () => {
		it('should execute single tool call', async () => {
			...
			expect(result.success).toBe(true);
			expect(result.toolsUsed).toEqual(['wp-agentic-admin/plugin-list']);
			expect(mockCallbacks.onToolStart).toHaveBeenCalledWith('wp-agentic-admin/plugin-list');
		...
		it('should chain multiple tools', async () => {
			mockEngine.chat.completions.create
				...
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "tool_call", "tool": "wp-agentic-admin/site-health", "args": {}}',
						},
					}],
				})
				...
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "final_answer", "content": "Site is healthy and you have 1 plugin."}',
						},
					}],
				});
	...
	describe('Repeated Call Detection', () => {
		it('should stop when same tool is called twice in a row', async () => {
			...
			expect(result.success).toBe(true);
			expect(result.toolsUsed).toEqual(['wp-agentic-admin/plugin-list']);
			expect(result.error).toBe('Repeated tool call detected (handled gracefully)');
	...
	describe('Max Iterations Safety', () => {
		it('should stop after 10 iterations', async () => {
			...
			mockEngine.chat.completions.create.mockImplementation(() => {
				callCount++;
				const tool = callCount % 2 === 0 ? 'wp-agentic-admin/plugin-list' : 'wp-agentic-admin/site-health';
		...
		it('should handle tool execution errors', async () => {
			...
			mockEngine.chat.completions.create
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "tool_call", "tool": "wp-agentic-admin/broken-tool", "args": {}}',
						},
					}],
				})
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "final_answer", "content": "Tool failed, but I handled it."}',
						},
					}],
				});
			...
			expect(result.success).toBe(true);
			expect(result.observations[0].result.error).toBe('Tool failed');
		});
		...
		it('should handle tool not found', async () => {
			...
			mockEngine.chat.completions.create
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "tool_call", "tool": "wp-agentic-admin/nonexistent", "args": {}}',
						},
					}],
				})
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "final_answer", "content": "Tool not found."}',
						},
					}],
				});
			...
			expect(result.success).toBe(true);
			expect(result.observations[0].result.error).toBe('Tool "wp-agentic-admin/nonexistent" not found');
		});
		...
		it('should request confirmation for tools that require it', async () => {
			...
			mockEngine.chat.completions.create
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "tool_call", "tool": "wp-agentic-admin/plugin-deactivate", "args": {"plugin": "test"}}',
						},
					}],
				})
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "final_answer", "content": "Plugin deactivated."}',
						},
					}],
				});
		...
		it('should cancel execution if user rejects confirmation', async () => {
			...
			mockEngine.chat.completions.create
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "tool_call", "tool": "wp-agentic-admin/plugin-deactivate", "args": {}}',
						},
					}],
				})
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "final_answer", "content": "Action cancelled."}',
						},
					}],
				});
	...
	describe('Conversational Mode (No Tools)', () => {
		it('should handle responses without tool calls', async () => {
			mockEngine.chat.completions.create.mockResolvedValueOnce({
				choices: [{
					message: {
						content: '{"action": "final_answer", "content": "A transient is temporary cached data in WordPress."}',
					},
				}],
			});

			const result = await reactAgent.execute('what is a transient?', []);
		...
		it('should handle "list plugins" correctly', async () => {
			mockEngine.chat.completions.create
				...
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "final_answer", "content": "You have 1 plugin installed: Test Plugin (active)."}',
						},
					}],
				});
		...
		it('should handle "my site is slow" with multi-tool chain', async () => {
			mockEngine.chat.completions.create
				...
				.mockResolvedValueOnce({
					choices: [{
						message: {
							content: '{"action": "final_answer", "content": "Your site is running WordPress 6.9 on PHP 8.3.29."}',
						},
					}],
				});
			...
			expect(result.success).toBe(true);
			expect(result.toolsUsed).toContain('wp-agentic-admin/site-health');
		});
	});
});
