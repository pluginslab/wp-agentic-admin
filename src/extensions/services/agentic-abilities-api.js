...
function registerAbility(id, config = {}) {
    if (!id || typeof id !== 'string') {
        console.error('[AgenticAbilitiesAPI] registerAbility: id must be a non-empty string');
        return false;
    }

    // Validate ID format (namespace/ability-name)
    if (!/^[a-z0-9-]+\/[a-z0-9-]+$/.test(id)) {
        console.error(`[AgenticAbilitiesAPI] registerAbility: Invalid ID format "${id}". Must be "namespace/ability-name".`);
        return false;
    }
...
function initializeFromPHPConfig() {
    ...
    console.log('[AgenticAbilitiesAPI] Initializing from PHP config:', Object.keys(phpAbilities));
...
function registerWorkflow(id, config = {}) {
    if (!id || typeof id !== 'string') {
        console.error('[AgenticAbilitiesAPI] registerWorkflow: id must be a non-empty string');
        return false;
    }

    // Validate ID format (namespace/workflow-name)
    if (!/^[a-z0-9-]+\/[a-z0-9-]+$/.test(id)) {
        console.error(`[AgenticAbilitiesAPI] registerWorkflow: Invalid ID format "${id}". Must be "namespace/workflow-name".`);
        return false;
    }

    if (!config.label) {
        console.error(`[AgenticAbilitiesAPI] registerWorkflow: workflow "${id}" must have a label`);
        return false;
    }

    if (!config.steps || !Array.isArray(config.steps) || config.steps.length === 0) {
        console.error(`[AgenticAbilitiesAPI] registerWorkflow: workflow "${id}" must have at least one step`);
        return false;
    }
    ...
    for (const step of config.steps) {
        if (!step.abilityId) {
            console.error(`[AgenticAbilitiesAPI] registerWorkflow: each step in "${id}" must have an abilityId`);
            return false;
        }
    ...
    try {
        ...
        console.log(`[AgenticAbilitiesAPI] Registered workflow: ${id} (${config.steps.length} steps)`);
        return true;
    } catch (error) {
        console.error(`[AgenticAbilitiesAPI] registerWorkflow error:`, error.message);
        return false;
    }
}
...
function exposeGlobalAPI() {
    ...
    console.log('[AgenticAbilitiesAPI] Global API exposed at wp.agenticAdmin (v1.1 with Workflows)');
}
