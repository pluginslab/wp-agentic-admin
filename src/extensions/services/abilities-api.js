...
class AbilitiesAPI {
        ...
        if (Array.isArray(abilities)) {
            abilities.forEach(ability => {
                ...
                if (abilityId) {
                    ...
                    console.log('[AbilitiesAPI] Cached ability:', abilityId, 'readonly:', ability?.meta?.annotations?.readonly);
                }
            });
        }
    ...
    async getAbility(namespace, name) {
        ...
        const ability = await this.request(`/abilities/${namespace}/${name}`);
    ...
    async executeAbility(namespace, name, input = {}) {
        ...
        console.log('[AbilitiesAPI] executeAbility:', abilityId, 'input:', input);
        console.log('[AbilitiesAPI] Cache has ability:', this.abilitiesCache.has(abilityId));
        ...
        if (this.abilitiesCache.has(abilityId)) {
            ...
            try {
                ...
                console.warn('[AbilitiesAPI] Could not fetch ability metadata, defaulting to POST:', err);
                ability = null;
            }
        }
        ...
        if (method === 'GET' || method === 'DELETE') {
            ...
            if (hasInput) {
                ...
                console.log(`[AbilitiesAPI] ${method} with input:`, endpoint);
            } else {
                console.log(`[AbilitiesAPI] ${method} without input:`, endpoint);
            }
