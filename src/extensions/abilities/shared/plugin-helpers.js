...
export function extractPluginParams(userMessage, actionKeywords = []) {
    ...
    if (window.wpAgenticAdmin?.pluginsList) {
        ...
        for (const [pluginFile, pluginData] of Object.entries(plugins)) {
            ...
            if (messageLower.includes(pluginName) || messageLower.includes(pluginSlug)) {
        ...
        if (quotedMatch) {
            ...
            for (const [pluginFile, pluginData] of Object.entries(plugins)) {
                const pluginName = pluginData.Name.toLowerCase();
                if (pluginName.includes(quoted) || quoted.includes(pluginName)) {
                    return { plugin: pluginFile };
                }
            }
        }
    ...
    if (actionKeywords.length > 0) {
        ...
        const patterns = [
            new RegExp(`(?:${keywordPattern})\\s+(?:the\\s+)?(?:plugin\\s+)?["']?([a-z0-9-_ ]+)["']?`, 'i'),
        ];
 ...
 */
export function formatPluginActionResult(result, defaultSuccessMessage = 'Action completed successfully.') {
    if (result.error) {
