 ...
 * @return void
 */
function wp_neural_admin_register_cron_list(): void {
	register_neural_ability(
		'wp-neural-admin/cron-list',
		// PHP configuration for WordPress Abilities API.
		array(
			'label'               => __( 'List Cron Events', 'wp-neural-admin' ),
			'description'         => __( 'List all scheduled WordPress cron events and their next run times.', 'wp-neural-admin' ),
			'category'            => 'sre-tools',
			'input_schema'        => array(
				'type'                 => 'object',
				'properties'           => array(
					'show_overdue' => array(
						'type'        => 'boolean',
						'description' => __( 'Highlight overdue cron events.', 'wp-neural-admin' ),
						'default'     => true,
					),
				),
			...
			'output_schema'       => array(
				...
				'properties' => array(
					...
					'total_events'  => array(
						'type'        => 'integer',
						'description' => __( 'Total number of cron events.', 'wp-neural-admin' ),
					),
					'overdue_count' => array(
						'type'        => 'integer',
						'description' => __( 'Number of overdue events.', 'wp-neural-admin' ),
					),
					'events'        => array(
						'type'        => 'array',
						'description' => __( 'List of cron events.', 'wp-neural-admin' ),
					),
				),
			),
			'execute_callback'    => 'wp_neural_admin_execute_cron_list',
		...
		array(
			...
			'initialMessage' => __( 'Fetching scheduled cron events...', 'wp-neural-admin' ),
		)
	);
}
 ...
 * @return array
 */
function wp_neural_admin_execute_cron_list( array $input = array() ): array {
	...
	if ( empty( $crons ) ) {
		return array(
			'success'       => true,
			'message'       => __( 'No cron events scheduled.', 'wp-neural-admin' ),
			'total_events'  => 0,
			'overdue_count' => 0,
			'events'        => array(),
		);
		...
		foreach ( $hooks as $hook => $events_data ) {
			foreach ( $events_data as $key => $event ) {
				...
				if ( $schedule_name ) {
					...
					$schedule_name = __( 'One-time', 'wp-neural-admin' );
				}
	...
	if ( $overdue > 0 && $show_overdue ) {
		$message = sprintf(
			...
			__( 'Found %1$d scheduled cron events. %2$d are overdue and may need attention.', 'wp-neural-admin' ),
			$total,
			$overdue
		);
	} else {
