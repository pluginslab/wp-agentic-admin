 ...
 * the 'wp_neural_admin_register_abilities' action.
 *
...
class Abilities {
	...
	private function load_ability_files(): void {
		$abilities_dir = WP_NEURAL_ADMIN_PLUGIN_DIR . 'includes/abilities/';
	 ...
	 * is ready. Third-party plugins should hook into 'wp_neural_admin_register_abilities'
	...
	public function register_core_abilities(): void {
		...
		if ( function_exists( 'wp_neural_admin_register_error_log_read' ) ) {
			wp_neural_admin_register_error_log_read();
		}

		if ( function_exists( 'wp_neural_admin_register_cache_flush' ) ) {
			wp_neural_admin_register_cache_flush();
		}

		if ( function_exists( 'wp_neural_admin_register_db_optimize' ) ) {
			wp_neural_admin_register_db_optimize();
		}

		if ( function_exists( 'wp_neural_admin_register_plugin_list' ) ) {
			wp_neural_admin_register_plugin_list();
		}

		if ( function_exists( 'wp_neural_admin_register_plugin_deactivate' ) ) {
			wp_neural_admin_register_plugin_deactivate();
		}

		if ( function_exists( 'wp_neural_admin_register_site_health' ) ) {
		 ...
		 * using the register_neural_ability() function.
		 *
		 * @since 1.0.0
		 *
		 * @example
		 ...
		 * });
		 */
		\do_action( 'wp_neural_admin_register_abilities' );
	}
}
